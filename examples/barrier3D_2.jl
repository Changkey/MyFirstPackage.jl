...
lb = example_d3q19()
vorticity = Observable(Vec3f.(reshape(_curl(momentum.(Ref(lb.config), lb.grid)),:)))
...
record(fig, joinpath(@__DIR__, "barrier.mp4"), 1:10; framerate = 10) do i
    for i=1:20
        println("==Step==")
        _step!(lb)
    end
    vorticity[] = Vec3f.(reshape(_curl(momentum.(Ref(lb.config), lb.grid)),:))
    println("==Fig==")
end